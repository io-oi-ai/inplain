/**
 * V26-C · edit_sheet Tool · 包装老 editSheetV2
 */
import { z } from "zod";
import { defineTool } from "../core/tool";
import { editSheetV2, type EditInput } from "@/lib/agents-v2/edit";
import type { AgentEvent as OldAgentEvent } from "@/lib/agents/types";

const EditSheetInput = z.object({
...
export const editSheetTool = defineTool({
  name: "edit_sheet",
  description: `修改一份现有 Plain Sheet(数据 dashboard)。
适合用户想要"加 KPI / 改 chart 类型 / 加排行榜 / 调 layout"时调用。
返回 RFC 6902 JSON Patch + 新 source。`,
  ...
  execute: async (args, ctx) => {
    ...
    if (!source.trim()) {
      return {
        kind: "error",
        code: "EDIT_SHEET_NO_SOURCE",
        message: "edit_sheet 缺少 current source:既没传 currentSource,也没能从 server 拉到 docId 对应的源。",
      };
    }
    ...
    try {
      const { source, patch, rationale } = await editSheetV2(input, (e) => {
        bridgeOldEventToCtx(e, ctx);
      });
      ctx.emit({ type: "doc", kind: "sheet", source });
      return { kind: "patch", ops: patch, rationale };
    } catch (e) {
      return {
        kind: "error",
        code: "EDIT_SHEET_FAILED",
